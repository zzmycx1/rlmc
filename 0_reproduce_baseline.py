...
def train(args, model, data_loader, optimizer):
    ...
    for i (batch_x, batch_y, batch_x_timestamp, batch_y_timestamp) in enumerate(data_loader):
        ...
        decoder_input = torch.cat([batch_y[:, :args.label_len, :], zeros_input], dim=1).to(device)  # [32, 72, 7]
        ...
        if "DLinear" in args.model:
            ...
            if args.output_attention:
                outputs = model(batch_x, batch_x_timestamp, decoder_input, batch_y_timestamp)[0]
            else:
                outputs = model(batch_x, batch_x_timestamp, decoder_input, batch_y_timestamp)
